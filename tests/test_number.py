...
from custom_components.zte_router_5g.number import ZTEPollingInterval, POLLING_INTERVAL_DESCRIPTION
from custom_components.zte_router_5g.const import DOMAIN, CONF_SCAN_INTERVAL
...
async def test_polling_interval_number(mock_coordinator, mock_config_entry):
    ...
    # FIX: Patch async_write_ha_state to prevent it from looking up the non-existent integration registry
    with patch.object(number, "async_write_ha_state"):
        with patch("asyncio.sleep", return_value=None): # Skip the 2s debounce wait
