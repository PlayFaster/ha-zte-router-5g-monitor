...
from custom_components.zte_router_5g.api import ZTERouterAPI, ZTEConnectionError, ZTEAuthError
...
def test_api_login_no_password():
    ...
    with patch.object(api, "get_LD", return_value="LD"), \
         patch.object(api, "get_version", return_value="VER"):
        with pytest.raises(Exception, match="No password provided"):
            api.login()
...
def test_api_reboot_success(mock_post):
    ...
    api = ZTERouterAPI("192.168.0.1", "admin", "password")
    api.stok = "stok=test"
    with patch.object(api, "login"), patch.object(api, "get_AD", return_value="test_ad"):
        mock_res = MagicMock()
...
def test_api_get_AD_new_gen():
    ...
    api = ZTERouterAPI("192.168.0.1", "admin", "password")
    with patch.object(api, "get_version", return_value="MC888_VER"):
        with patch.object(api, "get_RD", return_value="test_rd"):
