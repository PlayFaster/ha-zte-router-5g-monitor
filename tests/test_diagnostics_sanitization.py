...
# --------------------------------------------------------------------------
# A stale probe report in an upgraded entry
# --------------------------------------------------------------------------


async def test_a_stored_probe_report_is_stripped_from_the_entry(
    diagnostics_entry,
) -> None:
    """An entry created before the probe was retired still carries a report.

    The probe persisted its findings into the entry so they survived a
    restart, and nothing clears them. Publishing that back would put a stale
    report — 658 KB in the case this was measured on — into the download.
    """
    object.__setattr__(
        diagnostics_entry,
        "data",
        {
            **dict(diagnostics_entry.data),
            "delete_probe": {"completed": True, "note": FAKE_WAN_IP},
        },
    )
    ...
    assert FAKE_WAN_IP not in json.dumps(result, default=str)
