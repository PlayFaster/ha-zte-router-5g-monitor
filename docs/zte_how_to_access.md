...
| :-- | :-- | :-- |
| **HTML redirect** | Response URL contains `index.html`, or `Content-Type: text/html` with a body starting `<` | `_request`, HTML branch |
| **Unparsable JSON** | Body is not JSON at all | `_request`, parse branch |
| **Hollow JSON** | Valid JSON, HTTP 200, in which **every _authenticated_ value is an empty string** — or `result` is one of `session expired` / `unauth` / `fail`, or `status` is `fail` | `_classify_session` |
...
#### Not every key needs a session — and that is what makes detection possible

**Measured, MC7010 firmware `V1.0.0B03`, 2026-07-31.** Replaying an invalidated `stok`, five requested keys still answered:

| Batch                        | Keys still populated with a dead session |
| :--------------------------- | :--------------------------------------- |
| `_CORE_PARAMS` (80 keys)     | `imei`, `model_name`, `wa_inner_version` |
| `_EXTENDED_PARAMS` (36 keys) | `opms_wan_mode`, `opms_wan_auto_mode`    |

This gives a clean three-way read, and `_classify_session()` implements it:
...
| `not_ready` | everything blank, unauthenticated included | reachable, nothing to report yet |
| `undecidable` | no unauthenticated key was requested | fall back to the older all-values rule |

The unauthenticated keys are the reachability proof. Because the router is plainly answering, blankness in the authenticated keys cannot be a connectivity problem — it is the session. And when even those are blank, the router is booting, not expired, so re-logging in would not help; that path raises `ZTEConnectionError` and the coordinator holds last known values.

`not_ready` was observed directly: for roughly 35 seconds after a reboot the router refuses connections, then answers with everything blank before the authenticated data returns.

> [!WARNING] **Never write a session test as a property of the whole response.** The rule was once "every value is an empty string". That is a property of _what was requested_, not of the session, and it broke silently twice. First it named batch-poll keys, so it could not fire on an SMS response. Then `imei`, `model_name` and `wa_inner_version` were added to the core batch, making the condition unsatisfiable — a dead session became undetectable on the main poll, was scored a clean success, and every entity published `unknown` with a green health sensor and nothing in the log.
>
> Both times the rule had no stated precondition, so nothing could notice it had stopped being valid. `test_every_batch_carries_both_classes` now asserts that precondition: each batch must carry keys of **both** classes. If a batch edit trips it, keep a key of the missing class or change the classifier — do not delete the assertion.

On any of the three signatures, the client re-logs in and retries **once** (`_retry=False` on the retry, so a genuinely rejected credential surfaces as `ZTECredentialsError` rather than looping).

#### Which auth failure asks the user to re-authenticate
...
**Logins are serialized.** Two logins started together from one client had one refused with result `3` in each of three pairs; ten pairs run one after the other were all accepted. `login()` holds `_login_lock`, and a caller that waited logs in again rather than reusing the session just made.
...
It also interacts with expiry detection, and the interaction is sharper than it looks. A healthy MC7010 answers roughly **25 of 80** core keys empty — unused APN slots, the 5G metrics while on LTE — so an empty-key _count_ carries almost no information. Only the delta against a settled baseline identifies a key that has genuinely gone missing, which is why attended `[G]` takes a baseline before rebooting rather than counting blanks.

The danger this was once thought to pose — a batch of only-unpopulated keys being indistinguishable from a dead session — is not the one that occurred. The reverse happened: the batch gained keys that answer **without** a session, which made the all-values-empty condition unsatisfiable and disabled expiry detection outright. See [Session expiry](#session-expiry--three-different-signatures).
...
### `APN_PROC_EX` is all-or-nothing, and asymmetric
...
`set_apn_mode()` therefore sent a payload the router refused **for the manual direction in every release**, and the entity was never obviously dead because the auto direction worked. It was masked further by `set_apn()`, which already sent the complete form: choosing an **APN Profile** works, and that form carries `apn_mode=manual`, so the mode flipped as a side effect.
...
In **auto** mode the router connects with the network-provided default APN and `apn_index` merely retains the last _manual_ choice. It describes a stored preference, not what the device is doing. **`wan_apn` — the `Network APN` sensor — is the authoritative value.**

This is not cosmetic. Building a switch-to-manual payload from `apn_index` would activate a different APN than the one currently working, which is a worse failure than the refusal it was written to fix. `set_apn_mode()` therefore resolves the profile by matching `wan_apn` against each profile's APN field, case insensitively (the router reports `3FWA.ie` for a profile storing `3fwa.ie`).

Three consequences worth knowing before touching this code:

- **The default APN need not exist in the manual list at all.** On the reference device it does only because a matching profile was added by hand. Where nothing matches, there is no honest answer to "which profile?", so the switch to manual is **refused** with a message pointing at the APN Profile selector — which sets mode and profile together, and is the route the router actually supports.
- **Choosing an APN Profile always forces `apn_mode=manual`**, because `set_apn()`'s payload carries it. The mode appears to follow the profile; it is really a side effect. This is what masked the broken `set_apn_mode()` for so long.
- **The `Default` profile stores an empty APN field.** Selecting it in manual mode leaves `wan_apn` blank, so `Network APN` reads as unknown. That is correct and matches the router's own page — it is not a fault, and `apn_index` is authoritative in manual mode precisely so this case still resolves.

### Network mode values and their web-UI names

`SET_BEARER_PREFERENCE` takes the values on the left; the router's own page shows the names on the right. Reported by the project owner from the web UI — the API never exposes these labels, so this mapping is **not** independently verifiable from traffic:
