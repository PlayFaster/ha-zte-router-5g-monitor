...
| **Hollow JSON** | Valid JSON, HTTP 200, but `network_type` and `signalbar` are both empty strings — or `result` is one of `session expired` / `unauth` / `fail`, or `status` is `fail` | `api.py:219` |
...
Independently of the above, if more than **150 seconds** have passed since the last successful request, the client discards `stok` and re-logs in before sending (`api.py:112`). The router expires idle sessions somewhere above this, and preempting it is cheaper than a failed request plus a retry.
