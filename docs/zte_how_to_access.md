...
> [!WARNING]
> **Never write a session test as a property of the whole response.** The rule was once "every value is an empty string". That is a property of _what was requested_, not of the session, and it broke silently twice. First it named batch-poll keys, so it could not fire on an SMS response. Then `imei`, `model_name` and `wa_inner_version` were added to the core batch, making the condition unsatisfiable — a dead session became undetectable on the main poll, was scored a clean success, and every entity published `unknown` with a green health sensor and nothing in the log.
>
