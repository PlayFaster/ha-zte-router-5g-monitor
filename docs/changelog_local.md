...
- **Network Mode Selection offers the router's own options.** They are read from `AUTO_MODES` in the model-specific `js/config/<DEVICE>/config.js`, the list the router's network mode page builds its menu from, and kept in the router's order. The fixed `4G_AND_5G`, `LTE_AND_5G`, `Only_5G` and `Only_LTE` are removed. A current value outside the list, such as the MC888 Pro's `WL_AND_5G`, is added while it is current, so the select shows it instead of unknown (issue #79). Until the list is learned, the select offers the current value alone and refuses a change with `select_options_not_learned`; with no current value either, it is unavailable. A value outside the learned list is refused with `select_option_not_offered` and never written.
- **The device profile reads the model-specific files.** `parse_profile` adds `device_config` (every option list and flag of the device `config.js`) and `menus` (each device menu's page paths). `PROFILE_VERSION` goes from 1 to 2, so a profile stored by 3.4.3, which has no `AUTO_MODES`, is discarded and learned again once after the upgrade. `device_config.auto_modes` is named in `unlearned` where the list does not parse.
- **The crawl fetches all three device menus.** `web_sources._DEVICE_MODULES` gains `menu_4ggateway.js` and `menu_pppoe.js` beside `config.js` and `menu_bridge.js`: on a CPE, `main.js` picks one of the three from `opms_wan_mode`, and the crawl runs without a login and does not read the mode.
- **Total Data, Data Limit Switch and Total Connected Time are enabled by default** on new installs. An existing install keeps its registry setting; `reset_entities` applies the new defaults on request.
- **Total Connected Time no longer records long-term statistics,** matching the other duration sensors. An install that enabled it under 3.4.3 may see Home Assistant list its statistics as orphaned and offer to delete them; deleting them is safe.
- **Connection Duration, Uptime Duration and Total Connected Time suggest hours and set no display precision,** so the frontend shows hours and minutes, for example `20 hours 15 minutes`. See Checked Live for what reaches an existing entity.
- **The diagnostics discovery probes reported and generated spellings.** `EXPECTED_NAMES` gains 12 names used by `tpoechtrager/ZTE-Web-Script` and `nicjac/python-zte-mc801a`. A new `GENERATED_NAMES`, built by rule in `known_names.py`, holds 181 names: every 5G suffix under each of `5g_`, `z5g_`, `Z5g_`, `nr5g_`, `nr_` and `Nr_` (125), `network_` before each known signal and cell name (53), and the bare form of each name known only with `flux_` (3). `Nr_`, from names already in `EXPECTED_NAMES`, was added to the plan's five prefixes during implementation. The rule that names are added by observation only is changed by owner decision and recorded in the module docstring. The poll is unchanged.
- **`names_from_union_only` counts every shared list.** It counted `KNOWN_NAMES` alone, so the `EXPECTED_NAMES` probed since 3.3.9 were never counted; they were probed. It is now counted from the names probed, and `names_from_union_by_source` gives probed and answered counts for `known`, `expected` and `generated`.
- **The diagnostics download publishes the model-specific files as values:** under `device_profile.device`, the config file read, `AUTO_MODES`, every option list and flag, the menus read, and the page list of the menu for the current `opms_wan_mode`.

### Fixed

- **Enhanced Diagnostics Redaction**: `_sweep` now tokenizes `+` followed by 8 to 15 digits, and a whole value of 10 or 11 digits starting with `0`, as `phone-N`, in polled data and discovery values alike; no counter or identifier in any download held starts with `0`. `_DENY_NAME_RE` gains `phone` and `sms_number`, which covers the four other `*_alert_sms_number` names seen.
- **Entities did not re-render after a background profile learn** (found in the live check). The profile is learned after the first poll; the select wrote its state three seconds earlier and, with polling paused, kept offering the current value alone until the next poll. `async_learn_profile` now calls `async_update_listeners()` once the profile is learned.

### Corrected

- **3.4.2-dev8 stated that the minutes change reached the existing entities.** The registry shows it did not: the stored suggested unit of Uptime Duration and Connection Duration was still `h`, set at registration in 3.2.5, and the minutes display came from a unit the owner had set by hand. The dev8 entry is corrected.

### Checked Live on the MC7010

| Check | Result |
| :-- | :-- |
| Restart onto dev1 with a version 1 profile stored | Profile learned from 19 files, nothing unlearned; the select offered `4G_AND_5G`, `LTE_AND_5G`, `Only_5G`, `Only_LTE` after the listener fix |
| Second restart | Version 2 profile loaded from storage; no relearn; options present at once |
| Write `4G_AND_5G`, the current mode | Login, write and forced refresh; accepted, nothing changed on the router |
| Select `WL_AND_5G` | Refused by Home Assistant's own option check before the integration |
| Existing entity registry, manual unit and precision cleared first | Uptime Duration and Connection Duration display `h`, their stored suggested unit from registration; Total Connected Time keeps `min`, its stored suggested unit from 3.4.3. A changed suggested unit does not reach an existing entity. Home Assistant stored a display precision of 2 on all three |
| Total Connected Time state class | None on the existing entity |
| Diagnostics check | 65 of 65, two runs; device menus fetched, 47 web files; `names_from_union_only` 319, of which 177 generated and 35 expected |
| Generated names | `nr5g_band_lock` answered, `1,3,7,20,28,38,77,78`; not previously known |
| Phone tokens | Only the SMS sender, as before; no other value matched |

### Tests

- `test_network_mode_options.py`: `AUTO_MODES` and every option list and flag parse from the served text; the general `config.js` is not taken for the device file; a list that does not parse is not learned; menus list each page once; the menu follows `opms_wan_mode` as `main.js` chooses it; the download summary; the crawl asks for all three menus; the select's four states and both refusals; a version 1 profile is discarded and the relearn offers the list; entities re-render after a learn; the new defaults and display units.
- `test_diagnostics_vocabulary.py`: international and national numbers are tokenized under any key and counters are not; a national number inside text is not matched; phone fields are withheld by name; the 12 reported names; the generated set's rules, pinned at 181, and `nr5g_band_lock` in it.
- `test_diagnostic_capture.py`: generated names are probed and counted by source.
- Eleven mutations, one per change, each disabling it: every one fails its tests; restored, all pass.
- Updated where they encoded the old behavior: the suggested unit and precision of the duration sensors; the device-profile fixtures, which carry the write-path files only, now expect `device_config.auto_modes` unlearned; the lifecycle helper sets `coordinator.data`; the artefact fixture carries `names_from_union_by_source`; the network mode outage test supplies a learned list.

### Validation

To be recorded after `Fix and Validate All`.
...
- **Uptime Duration and Connection Duration display in minutes with one decimal place.** They displayed in hours with one decimal place, so the shown value changed only every 6 minutes while the entity changed at every poll. The user found this; it had been diagnosed as polling faults first. Neither sensor has a state class, and that is unchanged. _Corrected in 3.4.4-dev1: this entry stated that the change reached the existing entities in the devcontainer. It did not; the registry kept the suggested unit `h` from registration, and the minutes display came from a unit set by hand._
...
| Duration display | Minutes, on the existing entities, from a unit set by hand (corrected in 3.4.4-dev1) |
...
- **`scripts/diag_check.py` was overwriting the live observation stores.** The script builds its own coordinator so that a run cannot disturb the entry serving the user's entities, and `_StubEntry` says so. It nonetheless built that coordinator on the _live_ entry id, read from `core.config_entries`, and five stores are named from the entry id: `_history`, `_observed`, `_profile` and `_uptime`, along with the repair issue ids.
...
- **A digest name table, because resolution by definition is not enough.** `cookWithRequest` on the Pro forwards one hop to `SHA256` in `js/util.js`, which resolves. `hex_md5` on the MC7010 is defined in **no returned source** - it lives under `js/lib/`, which the crawl fetches and deliberately excludes - so a parser that read only definitions would have learned nothing from the one device this project can put a write on.
  ...
  A name maps to a digest **only where a real device's accepted token proves what it computes**. An unrecognized name is _unresolved_, which is a state and not a guess: inventing an entry would put a well-formed wrong token on the wire, and a wrong token is refused with nothing said about why.
...
- **The session flag is now the one the device nominates.** No firmware has an opinion about which readings a third-party client should watch, so the witness keys remain selected from what the device populates. What every firmware does name is the flag its _own_ client trusts, and both devices decide identically: `"ok" == loginfo`, read on its own. That is what the pre-write check has read since `[3.3.25-dev1]`, and it is now read from the script rather than written here.
...
- **Item 29 is dropped, and the plan records why.** It proposed deriving the token again with the single-operand form and sending a new request when the two-operand token is refused. There is no second form left to fall back to: `get_ad` computes `hash(version + cr_version)`, and a device that does not answer `cr_version` appends an empty string - so on that class of device the derivation already _is_ the single-operand form, bit for bit, and the retry would re-send a token identical to the one just refused. On a device that does answer it, the single-operand token is simply wrong.
...
- **The two names a write's token is derived from resolve across spellings**, as every other read in this integration already did. **The alias resolves the read and never the operand** - the token hashes the _value_, and a spelling that silently became the operand would produce a well-formed wrong token, which is a write refused with nothing to say why.
...
- **A request is now a value.** `_Call` carries the method, path, params, body, caller headers, timeout, `authenticated`, `requested`, `classify` and the two recursion flags. `_request` keeps its signature byte-identical - every caller and every test sees what it always did - and its body is now the construction of a `_Call` and a delegation.
  ...
  **Assembled headers are deliberately not on it.** `Referer` and the session `Cookie` are built per attempt from live state, because a replay follows a re-login and must carry the _new_ cookie. Caching them would replay the dead one: an authentication failure presenting as a router fault, on the recovery path, where it is least visible.
...
- **No write path changed in this release.** Detection, recovery and the block list are as `[3.3.25-dev3]` left them, and the limitation recorded there stands: a browser login took the session, the check detected it within a second, logged in again and confirmed, and the write was still refused. Contention no pre-write check can prevent. What phase 2 adds is that the next occurrence explains itself in a download.
...
- **Validate Bump**: Update `ruff` from 0.16.5 to 0.16.6
