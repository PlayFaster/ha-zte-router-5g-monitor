...
- **ZTE SMS API Requirements**: Interfacing with the ZTE router SMS API requires specific encoding and security handling:
  - *Message Encoding*: Messages must be hex-encoded using UTF-16BE format (`encode("utf-16-be").hex()`). If not encoded, characters will be garbled or rejected by the modem.
  - *Recipient Numbers*: Phone numbers must be URL-escaped.
  - *Timestamp format*: The router demands a semicolon-delimited date-time string of the format `yy;mm;dd;HH;MM;SS;+0` matching the time the message is sent.
  - *AD Security Parameter*: State-changing actions (like reboot, send_sms, delete_sms) must pass an `AD` security parameter. This is dynamically generated by hashing (SHA256 in uppercase for newer MC888/MC889 modems, MD5 for older ones like MC7010) the router version string combined with a transient `RD` value fetched from the router API.
  - *Bulk/Partial Deletions*: Bulk deletion is optimized by passing a semicolon-joined string of IDs (e.g. `"1;2;3"`) to the `msg_id` parameter of `DELETE_SMS`, allowing the integration to delete multiple messages in a single POST request rather than making sequential calls.
