...
- **`api.py` (`ZTERouterAPI`)** — stateless-ish async client for the `goform` API. Key behaviors that are easy to break:
  ...
  - **`login()` returns `None`.** There is nothing to hand back when a successful login may issue no token; read `api.stok` and `api.session_active`. `_extract_stok()` looks for the token in `r.cookies`, then the raw `Set-Cookie` headers case-insensitively, then the cookie jar, then the response body — `SimpleCookie` morsel names are case-sensitive and it drops unparsable headers, and a token set on an intermediate `302` reaches only the jar. The jar is read **only** inside `_attempt_login`, after `login()` has emptied it, so a `stok` found there was set by that request and never left by the session just ended.
